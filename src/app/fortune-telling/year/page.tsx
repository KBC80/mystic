...
import { TrendingUp, Home, CalendarIcon, Sparkles, Wand2 } from 'lucide-react';
...
type YearlyFortuneFormValues = z.infer<typeof formSchema>;
...
export default function YearlyFortunePage() {
  ...
  const [targetFieldNameForHanja, setTargetFieldNameForHanja] = useState<FieldPath<YearlyFortuneFormValues> | null>(null);

  const form = useForm<YearlyFortuneFormValues>({
  ...
  const handleOpenHanjaModal = (fieldName: FieldPath<YearlyFortuneFormValues>) => {
  ...
  async function onSubmit(values: YearlyFortuneFormValues) {
    ...
    router.push(`/fortune-telling/year/result?${queryParams}`);
  }
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Button type="submit" disabled={isSubmitting} className="w-full md:w-auto bg-accent hover:bg-accent/90 text-accent-foreground">
                {isSubmitting ? <LoadingSpinner size={20} /> : "올해 운세 보기"}
              </Button>
            </form>
          </Form>
        </CardContent>
