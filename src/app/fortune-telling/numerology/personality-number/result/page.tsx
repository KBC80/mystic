...
function PersonalityNumberResultContent() {
  ...
  useEffect(() => {
    ...
    getPersonalityNumberAnalysis({ name })
      ...
      .catch(err => {
        ...
        setError(err instanceof Error ? err.message : "성격수 분석 결과를 가져오는 중 알 수 없는 오류가 발생했습니다.");
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = image;
      const nameForFile = inputName.replace(/\s*\(.*\)\s*$/, "").trim() || "성격수";
      link.download = `${nameForFile}_성격수${result?.personalityNumber}_결과.png`;
      link.click();
    } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-200px)] p-6">
        ...
        <p className="mt-4 text-lg text-muted-foreground break-words">당신의 외적인 모습을 분석하고 있습니다...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 py-6 flex flex-col flex-1">
      <div ref={resultAreaRef}>
        ...
        <SectionCard title="외적으로 드러나는 특성" icon={Sparkles} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.externalTraits}</p>
        </SectionCard>

        <SectionCard title="첫인상" icon={MessageSquare} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.firstImpression}</p>
        </SectionCard>
        
        <SectionCard title="사회적 스타일" icon={Users} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.socialStyle}</p>
        </SectionCard>

        <SectionCard title="발전 방향 및 조언" icon={Lightbulb} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.howToDevelop}</p>
        </SectionCard>
        
        <Card className="mt-6 shadow-md">
          <CardHeader>
            <CardTitle className="text-xl text-primary flex items-center gap-2">
              <Gift className="h-5 w-5"/> 성격수 행운의 숫자
            </CardTitle>
          </CardHeader>
          <CardContent>
...
export default function PersonalityNumberResultPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-200px)] p-6">
        <LoadingSpinner size={48} />
      ...
      <PersonalityNumberResultContent />
    </Suspense>
  );
}
