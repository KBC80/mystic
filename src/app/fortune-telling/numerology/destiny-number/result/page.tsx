...
import { getDestinyNumberAnalysis, type DestinyNumberOutput } from '@/ai/flows/destiny-number-flow';
import { getTarotImageForNumerology } from '@/lib/numerology-utils';
import html2canvas from 'html2canvas';
import { Home, Star, Sparkles, Sigma, Briefcase, Heart, Lightbulb, Gift, RotateCcw, Share, AlertTriangle, BookOpen } from 'lucide-react';
...
function DestinyNumberResultContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const resultAreaRef = useRef<HTMLDivElement>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<DestinyNumberOutput | null>(null);
  ...
  useEffect(() => {
    ...
    getDestinyNumberAnalysis({ name, birthDate })
      .then(analysisResult => {
        setResult(analysisResult);
        if (analysisResult?.destinyNumber) {
          setTarotImage(getTarotImageForNumerology(analysisResult.destinyNumber));
        }
      })
      .catch(err => {
        console.error("운명수 분석 결과 오류:", err);
        setError(err instanceof Error ? err.message : "운명수 분석 결과를 가져오는 중 알 수 없는 오류가 발생했습니다.");
      })
      .finally(() => {
        setIsLoading(false);
      });
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = image;
      const nameForFile = inputName.replace(/\s*\(.*\)\s*$/, "").trim() || "운명수";
      link.download = `${nameForFile}_운명수${result?.destinyNumber}_결과.png`;
      link.click();
    } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-200px)] p-6">
        ...
        <p className="mt-4 text-lg text-muted-foreground break-words">당신의 운명수를 분석 중입니다...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-4">
        ...
        <Button onClick={() => router.push('/fortune-telling/numerology/destiny-number')} variant="outline" className="mt-4">
          다른 정보로 분석
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-6 py-6 flex flex-col flex-1">
      <div ref={resultAreaRef}>
        <Card className="shadow-lg border-primary/30 bg-primary/5 dark:bg-primary/10">
          <CardHeader className="pb-4">
            <CardTitle className="text-3xl text-primary flex items-center gap-3">
              <Star className="h-8 w-8" /> {inputName}님의 운명수: {result.destinyNumber}
            </CardTitle>
          ...
          {tarotImage && (
            <CardContent className="flex flex-col items-center justify-center pt-2 pb-4">
              <p className="text-sm font-medium text-secondary-foreground mb-1">운명수 {result.destinyNumber}의 타로 상징: {tarotImage.name}</p>
              <Image
        ...
        <SectionCard title="재능 및 직업" icon={Briefcase} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.talentsAndCareer}</p>
        </SectionCard>
        
        <SectionCard title="관계 및 연애" icon={Heart} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.relationships}</p>
        </SectionCard>

        <SectionCard title="삶의 목적 및 조언" icon={Lightbulb} className="mt-6">
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{result.lifePurposeAndAdvice}</p>
        </SectionCard>
        
        <Card className="mt-6 shadow-md">
          <CardHeader>
            <CardTitle className="text-xl text-primary flex items-center gap-2">
              <Gift className="h-5 w-5"/> 운명수 행운의 숫자
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <CardFooter className="mt-4 pt-6 border-t flex flex-col sm:flex-row items-center justify-center gap-4">
        <Button onClick={() => router.push('/fortune-telling/numerology/destiny-number')} variant="outline" className="shadow-sm hover:shadow-md transition-shadow w-full sm:w-auto">
...
export default function DestinyNumberResultPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col justify-center items-center min-h-[calc(100vh-200px)] p-6">
        <LoadingSpinner size={48} />
