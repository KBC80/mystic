...
import { Star, Home, Sigma, Wand2, Sparkles, CalendarIcon } from 'lucide-react';
...
type DestinyNumberFormValues = z.infer<typeof formSchema>;
...
export default function DestinyNumberPage() {
  ...
  const [targetFieldNameForHanja, setTargetFieldNameForHanja] = useState<FieldPath<DestinyNumberFormValues> | null>(null);

  const form = useForm<DestinyNumberFormValues>({
  ...
  const handleOpenHanjaModal = (fieldName: FieldPath<DestinyNumberFormValues>) => {
  ...
  async function onSubmit(values: DestinyNumberFormValues) {
    ...
    router.push(`/fortune-telling/numerology/destiny-number/result?${queryParams}`);
  }
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground break-words">
            당신의 이름과 생년월일에 담긴 운명수를 통해 타고난 성향, 잠재력, 삶의 방향을 알아보세요.
          </p>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Button type="submit" disabled={isSubmitting} className="w-full md:w-auto bg-accent hover:bg-accent/90 text-accent-foreground">
                {isSubmitting ? <LoadingSpinner size={20} /> : "운명수 확인하기"}
              </Button>
            </form>
          </Form>
        </CardContent>
