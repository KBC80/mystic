...
import { Home, Sparkles, User, CalendarDays, Clock, Info, Palette, BookOpen, TrendingUp, Mic, Gem, Filter, CheckCircle, AlertTriangle, RotateCcw, PieChartIcon, Award, BarChart3, HelpCircle, Gift, Share } from 'lucide-react';
...
function NameInterpretationResultContent() {
  ...
  const handleDownloadImage = async () => {
    if (!resultAreaRef.current || !result?.basicInfoSummary?.koreanName) {
      console.error("결과 영역 또는 이름 정보를 찾을 수 없습니다.");
    ...
    try {
      ...
      link.href = image;
      link.download = `${result.basicInfoSummary.koreanName.replace(/\s*\(.*\)\s*$/, "").trim()}_이름풀이결과.png`;
      link.click();
    } catch (error) {
  ...
  return (
    <div className="space-y-6 py-6 flex flex-col flex-1">
      <div ref={resultAreaRef}>
        ...
        <SectionCard title="기본 정보 요약" icon={Info} className="bg-card mt-6">
        ...
        <SectionCard title="종합 점수 및 평가" icon={Award} className="bg-card mt-6">
        ...
        <SectionCard title="수리길흉 분석 (원형이정 4격)" icon={TrendingUp} className="bg-card mt-6" cardDescription={da.suriGilhyungAnalysis.introduction}>
        ...
        <SectionCard title="오행 및 음양 상세 분석" icon={Palette} className="bg-card mt-6" cardDescription="이름의 소리, 글자 모양, 한자 뜻에 담긴 오행과 음양의 조화를 분석합니다.">
        ...
        <SectionCard title="한자 필터링 및 종합조언" icon={Filter} className="bg-card mt-6">
