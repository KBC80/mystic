...
'use server';
/**
 * @fileOverview 사용자의 이름을 기반으로 성격수(Personality Number)를 계산하고 해석합니다.
 *
...
export async function getPersonalityNumberAnalysis(nameInput: { name: string }): Promise<PersonalityNumberOutput> {
  const personalityNumber = calculatePersonalityNumber(nameInput.name);
  return personalityNumberAnalysisFlow({ name: nameInput.name, personalityNumber });
}
