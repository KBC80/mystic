...
const LottoSetSchema = z.object({
  numbers: z
    ...
    .length(6)
    .describe('추천된 로또 번호 6개입니다 (1-45 사이). 각 세트의 번호는 최대한 중복되지 않도록 다양하게 구성해주세요.'),
...
결과 형식:
1.  **추천 로또 번호 조합 (3세트)**:
    *   각 세트마다 6개의 번호를 제시하고, 왜 이 번호 조합이 사용자에게 길운을 가져다줄 수 있는지에 대한 심층적이고 설득력 있는 해설을 포함해야 합니다. (예: 사주 오행의 조화, 이름의 수리적 의미, 특별한 날짜와의 연관성 등) 각 세트의 번호는 최대한 다양하게, 특정 숫자에 편중되지 않도록 구성해주세요.
2.  **전반적인 행운 조언**:
