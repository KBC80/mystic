...
'use server';
/**
 * @fileOverview 사용자의 이름과 생년월일을 기반으로 생명수(Soul Urge Number)를 계산하고 해석합니다.
 *
...
export async function getSoulUrgeNumberAnalysis(input: { name: string, birthDate: string }): Promise<SoulUrgeNumberOutput> {
  const soulUrgeNumber = calculateSoulUrgeNumber(input.name);
  return soulUrgeNumberAnalysisFlow({ name: input.name, birthDate: input.birthDate, soulUrgeNumber });
}
...
7.  **행운의 숫자 (luckyNumbers)**: **이 생명수 {{{soulUrgeNumber}}} 분석과, 이름 "{{{name}}}" 및 생년월일 "{{{birthDate}}}"의 기운을 종합적으로 고려하여** 1부터 45 사이의 **서로 다른** 행운의 숫자 3개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
