...
const GetWeeklyHoroscopeOutputSchema = z.object({
  ...
  luckyNumbers: z
    ...
    .length(3)
    .describe('이 별자리 운세 분석을 바탕으로 한 1에서 45 사이의 이번 주 서로 다른 행운의 숫자 세 개를 다양하게, 특정 숫자에 편향되지 않도록 추천합니다.'),
});
...
8.  **행운의 숫자 (luckyNumbers)**: **이 별자리 운세 분석을 바탕으로** 1부터 45 사이의 이번 주 **서로 다른** 행운의 숫자 3개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
