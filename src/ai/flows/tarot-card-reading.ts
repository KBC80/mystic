...
const TarotCardReadingOutputSchema = z.object({
  ...
  luckyNumbers: z
    ...
    .length(3)
    .describe('1에서 45 사이의 **서로 다른** 행운의 숫자 세 개를 **무작위로 다양하게, 특정 숫자에 편향되지 않도록** 추천합니다.'),
});
...
마지막으로, 이 타로 리딩을 바탕으로 사용자에게 특별한 행운을 가져다 줄 1부터 45 사이의 **서로 다른** 숫자 3개를 **무작위로 다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
