...
const TarotCardReadingInputSchema = z.object({
  ...
  card4: z.string().describe('사용자가 선택한 네 번째 타로 카드입니다.'),
  card5: z.string().describe('사용자가 선택한 다섯 번째 타로 카드입니다.'),
});
...
const TarotCardReadingOutputSchema = z.object({
  ...
  card4Interpretation: z
    .string()
    .describe('네 번째 타로 카드에 대한 상세한 해석입니다.'),
  card5Interpretation: z
    .string()
    .describe('다섯 번째 타로 카드에 대한 상세한 해석입니다.'),
  overallAdvice: z.string().describe('카드 리딩을 바탕으로 한 전반적인 조언입니다.'),
  luckyNumbers: z
    .array(z.number().int().min(1).max(45))
    .length(3)
    .describe('이 타로 리딩을 바탕으로 한 1에서 45 사이의 **서로 다른** 행운의 숫자 세 개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천합니다.'),
});
...
사용자가 선택한 카드는 다음과 같습니다: {{{card1}}}, {{{card2}}}, {{{card3}}}, {{{card4}}}, {{{card5}}}.
...
선택된 5장의 각 카드에 대한 상세한 해석을 반드시 포함해주세요:
...
카드 4 ({{{card4}}}): [여기에 {{{card4}}}에 대한 구체적이고 심층적인 해석. 사용자의 질문과 연결하여 설명합니다.]
카드 5 ({{{card5}}}): [여기에 {{{card5}}}에 대한 구체적이고 심층적인 해석. 사용자의 질문과 연결하여 설명합니다.]

다섯 카드의 조합을 통해 드러나는 전반적인 흐름과 메시지를 종합적으로 분석하고, 이를 바탕으로 사용자에게 구체적이고 실용적인 조언을 제공해주세요. 마치 오랜 경험에서 우러나오는 따뜻하면서도 현실적인 조언처럼, 사용자가 앞으로 나아갈 길을 밝혀주는 지침을 내려주세요. 사용자의 현재 상황에 대한 심층적인 분석과 함께, 가능한 어려움과 그것을 극복할 방법에 대해서도 언급해주세요.

마지막으로, 이 타로 리딩을 바탕으로 사용자에게 특별한 행운을 가져다 줄 1부터 45 사이의 **서로 다른** 숫자 3개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
