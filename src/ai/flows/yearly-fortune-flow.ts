...
const GetYearlyFortuneOutputSchema = z.object({
  ...
  luckyNumbers: z
    ...
    .length(3)
    .describe('올해의 **서로 다른** 행운의 숫자 3개를 **무작위로 다양하게, 특정 숫자에 편향되지 않도록** 추천합니다 (1-45 사이).'),
...
8.  **행운의 숫자 (luckyNumbers)**: 1부터 45 사이의 올해의 **서로 다른** 행운의 숫자 3개를 **무작위로 다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
