...
const RuneReadingOutputSchema = z.object({
  ...
  luckyNumbers: z.array(z.number().int().min(1).max(45)).length(3).describe('룬 리딩을 바탕으로 한 **서로 다른** 행운의 숫자 세 개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천합니다 (1-45 사이).'),
});
...
3.  **행운의 숫자**: 이 룬 리딩을 바탕으로 사용자에게 특별한 행운을 가져다 줄 1부터 45 사이의 **서로 다른** 숫자 3개를 **다양하게, 특정 숫자에 편향되지 않도록** 추천해주세요.
