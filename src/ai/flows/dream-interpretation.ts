...
const DreamInterpretationOutputSchema = z.object({
  ...
  luckyNumbers: z
    ...
    .length(3)
    .describe('이 꿈 해석을 바탕으로 한 1에서 45 사이의 서로 다른 행운의 숫자 세 개를 다양하게, 특정 숫자에 편향되지 않도록 추천합니다.'),
});
...
const dreamInterpretationPrompt = ai.definePrompt({
  ...
  해석을 바탕으로 사용자에게 실질적인 도움이 될 수 있는 추가적인 주의사항, 꿈이 암시하는 잠재적인 행운, 그리고 **이 꿈 해석을 바탕으로** 1에서 45 사이의 **서로 다른** 행운의 숫자 세 개를 **다양하게, 특정 숫자에 편향되지 않도록** 제공해주세요.
